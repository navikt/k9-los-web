...
import { useAlleKoer, useAntallOppgaverIKøer } from 'fleromrade/api/avdelingslederQueries';
import NyKøDialog from 'fleromrade/avdelingsleder/koer/NyKøDialog';
...
const useBerikMedAntallOppgaverIndividuelt = (køArray: OppgavekøV3Enkel[]) => {
	const queries = useAntallOppgaverIKøer(køArray.map((kø) => kø.id));

	const isSuccess = queries.every((query) => query.isSuccess);
	const results: KøRad[] = queries.map((query, index) => ({
		...query.data,
		...køArray[index],
...
const BehandlingskoerIndex = () => {
	...
	const sortData = () => {
		const køer: KøRad[] = køerMedAntallOppgaver;
		if (!sort) return køer;
