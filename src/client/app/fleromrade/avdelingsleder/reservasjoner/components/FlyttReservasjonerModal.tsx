...
export const FlyttReservasjonerModal: FunctionComponent<OwnProps> = ({ open, closeModal, reservasjoner }) => {
	const { endre, isPending } = useEndreReservasjoner();
	const flyttReservasjoner = (endringer: Parameters<typeof endre>[0]) => endre(endringer, { onSuccess: closeModal });
	const { data: saksbehandlere = [], isLoading, error } = useSaksbehandlereForReservasjon();
